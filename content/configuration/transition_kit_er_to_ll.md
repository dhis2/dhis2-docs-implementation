...
[![A review of the line listing interface](https://img.youtube.com/vi/AwqLT35fjCg/0.jpg)](https://www.youtube.com/watch?v=AwqLT35fjCg)

1. [A review of the line listing interface](https://www.youtube.com/watch?v=AwqLT35fjCg)
...
| User interface/ Navigation 	| Groups dimensions by tab. Tabs include: Input (Event/Enrollment) Program (dates, status, org unit, data items) Your dimensions (OU group sets, etc.) Global dimensions The layout now persists on top of the line list, and you can drag and drop items as needed to modify the layout Can collapse items through “View” options Can change display density, font size, digit separator via the table options Can include the organisation unit hierarchy in output 	| Follows a very similar input method as other apps (what, where, when - data, periods, org units) followed by OU group sets Need to select between line list or pivot table type reports Rearrange column order by opening the layout tab Can collapse options via arrow selectors Can change display density, font size, digit separator (also present in DV) via table options Can include the organisation unit hierarchy in output 	|
